...
# Monitors X11 idle time via xprintidle and navigates browser back to the
# configured dashboard URL after timeout using Chrome DevTools Protocol (CDP).
...
# Browser control helper for CDP navigation
...
# Falls back to F5 reload if CDP navigation fails (e.g., Chromium still starting).
navigate_to_dashboard() {
  if python3 "$BROWSER_CONTROL" navigate "$DASHBOARD_URL" 2>/dev/null; then
    ...
    log_message "CDP navigation failed, falling back to F5 reload"
...
while true; do
  ...
  if [ "$IDLE_MS" -gt "$IDLE_TIMEOUT_MS" ]; then
    ...
    navigate_to_dashboard
    # Wait full timeout before checking again to avoid rapid-fire navigation
